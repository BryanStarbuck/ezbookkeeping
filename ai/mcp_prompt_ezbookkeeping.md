...
READ THE TREE FIRST. Before you categorise anything, call `{TOOL_PREFIX}get_category_tree`: it returns every group and sub-category, by major category, as one YAML document (`data.yaml`). Pick only paths it lists. If the right one is missing, propose creating it — never write a path you have not seen in the tree, and never invent a near-duplicate of a group that is already there. Read the tree again after you create categories.
...
For a large queue worked outside the chat — a script or a spreadsheet — use the id round trip instead: `{TOOL_PREFIX}export_transactions` with the fallback `category_ids` and `with_ids: true` returns a CSV whose first column is each row's database id and whose `New Category` / `New Counter Account` columns are empty; once they are filled, `{TOOL_PREFIX}set_transaction_categories_csv` takes the text back and makes the same previewed, undoable write. Rows left blank stay where they are.
...
CATEGORIES FROM THE STATEMENT FILES

When the operator's pipeline has already decided the categories, they are in each account's companion TSV next to the import: `Type` (the major category), `Category` (the group), `Sub Category`, and `FITID`, the OFX row's bank id. `{TOOL_PREFIX}set_import_categories` applies them to the rows that are already in the books — it imports nothing — by tracing each FITID through the plane's import record to its transaction. Pass `manifest_path` and `accounts` exactly as for the import.

Preview first (with `summary_only: true` for a big file) and read out `preview.byCategory`, `preview.skippedByReason` and `preview.unknownCategories`. Unknown paths are never created for you: show them, create them with `{TOOL_PREFIX}add_categories` after a yes, then preview again. Apply with the token and `max_changes` set to the preview's `update`. If `preview.remaining` is above 0, the file had more rows than one call changes: preview and apply again until it is 0. Preview once more at the end: `update` must be 0.

By default only rows still in a fallback category change, so a category the operator set by hand is never overwritten; `overwrite: true` changes that, and only the operator may ask for it. Rows counted `type_differs` are rows an earlier pass turned into transfers; they are correct as they are — say how many, do not try to convert them back. Rows counted `no_category` are rows the file left blank; they stay in the fallback and the work queue above is where they get decided.
...
"Fix the categories from my statement files." — `{TOOL_PREFIX}get_category_tree`, then `{TOOL_PREFIX}set_import_categories` as a preview per account group, `{TOOL_PREFIX}add_categories` for any unknown path the operator approves, then apply, repeat while `remaining` is above 0, and finish with a preview that shows 0 updates. Report per account how many rows moved, how many were already categorised, how many are transfers now, and how many the file left blank.

"Categorise my transactions." — `{TOOL_PREFIX}get_category_tree` to learn the operator's groups, then `{TOOL_PREFIX}list_uncategorized`, then decide a page of groups, `{TOOL_PREFIX}add_categories` for any missing path, and `{TOOL_PREFIX}set_transaction_categories` with one assignment per group, previewed with `summary_only`. Report what moved, fetch the queue again, and repeat until it is empty or only undecidable groups are left; list those for the operator.
